...
SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS null_year,
...
-- Total number of movies year wise
...
SELECT 
  COUNT( DISTINCT id) AS movie_count, year
FROM
    movie
WHERE
...
SELECT 
    genre, COUNT(movie_id) AS genre_count
FROM
    genre g
        INNER JOIN
...
SELECT 
    COUNT(a.movie_id) AS single_genre_movies_count
FROM
    (SELECT 
...
(
SELECT
    sum(r.total_votes) as german_movies_votes_count
FROM
    movie m
        INNER JOIN
    ratings r ON m.id = r.movie_id
WHERE m.country REGEXP 'Germany'
),
italian_movies AS
(
SELECT
    sum(r.total_votes) as italian_movies_votes_count
FROM
    movie m
        INNER JOIN
    ratings r ON m.id = r.movie_id
WHERE m.country REGEXP 'Italy'
)
SELECT german_movies_votes_count, italian_movies_votes_count,
IF(german_movies_votes_count > italian_movies_votes_count, 'German Movies get more votes than Italian movies','Italian Movies get more votes than German movies') as More_Votes
FROM german_movies gm, italian_movies im;
...
SELECT 
    COUNT(1) - COUNT(name) AS name_nulls,
    COUNT(1) - COUNT(height) AS height_nulls,
    COUNT(1) - COUNT(date_of_birth) AS date_of_birth_nulls,
    COUNT(1) - COUNT(known_for_movies) AS known_for_movies_nulls
FROM
    names;
...
(
SELECT genre, count(m.id) AS movie_count, RANK() OVER (ORDER BY COUNT(m.id) DESC) AS RNK
FROM genre g
INNER JOIN movie m
ON g.movie_id = m.id
...
SELECT 
    n.name AS director_name, COUNT(dm.movie_id) AS movie_count
FROM
    names n
        INNER JOIN
    ...
    genre g ON g.movie_id = dm.movie_id
        INNER JOIN
    ratings r ON r.movie_id = dm.movie_id
WHERE
    avg_rating > 8.0
        AND genre IN (SELECT 
            genre
...
WITH top_two_actors_tbl AS
(
SELECT  n.name AS  actor_name, COUNT(m.id) as movie_count, RANK() OVER (ORDER BY COUNT(m.id) DESC) as RNK
FROM
ratings r
INNER JOIN movie m
ON r.movie_id = m.id
...
SELECT 
    ...
    r.avg_rating,
    CASE
        WHEN r.avg_rating > 8.0 THEN 'Superhit movies'
        WHEN r.avg_rating BETWEEN 7.0 AND 8.0 THEN 'Hit movies'
        WHEN r.avg_rating BETWEEN 5.0 AND 7.0 THEN 'One-time watch movies'
        ELSE 'Flop movies'
...
WINDOW genre_w AS (ORDER BY genre ROWS UNBOUNDED PRECEDING);
...
CONCAT('$',REPLACE(REPLACE(worlwide_gross_income,'INR',''),'$','')) AS worlwide_gross_income,
-- worlwide_gross_income,
DENSE_RANK() OVER (PARTITION BY YEAR ORDER BY CAST(REPLACE(REPLACE(worlwide_gross_income,'INR',''),'$','') AS DECIMAL(10)) DESC) AS movie_rank
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
AND genre in (SELECT 
...
top_directors_movies AS
(
SELECT n.id AS director_id,
  n.NAME AS director_name,
  m.id AS movie_id,
  DATEDIFF(LEAD(m.date_published)
 OVER (partition BY n.id ORDER BY m.date_published),m.date_published) AS inter_movie_days,
